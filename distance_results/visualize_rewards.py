...
def load_points(json_path):
    ...
    for row in data.get("results", []):
        ...
        points.append({
            ...
            "normalizedCanonicalDistance": float(row.get("normalizedCanonicalDistance", 0)),
            "leftCanonicalFormSize": float(row.get("leftCanonicalFormSize", 0)),
            "rightCanonicalFormSize": float(row.get("rightCanonicalFormSize", 0)),
            "canonicalFormSize": float(row.get("canonicalFormSize", 0)),
            ...
            "normalizedRawAstDistance": float(row.get("normalizedRawAstDistance", 0)),
            "leftRawAstSize": float(row.get("leftRawAstSize", 0)),
            "rightRawAstSize": float(row.get("rightRawAstSize", 0)),
            "rawAstSize": float(row.get("rawAstSize", 0)),
...
def write_csv(points, output_dir):
    ...
    with path.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(handle, fieldnames=[
            ...
            "statusFolder",
            "distance",
            "normalizedCanonicalDistance",
            "leftCanonicalFormSize",
            "rightCanonicalFormSize",
            "canonicalFormSize",
            "predicateBodyLevenshteinDistance",
            "rawAstTreeDistance",
            "normalizedRawAstDistance",
            "leftRawAstSize",
            "rightRawAstSize",
            "rawAstSize",
            "candidateReward",
