"""Bounded checks for the v2.17 profile adapter and frozen dispatcher.
...
import fifth_obligation_replays as replay
...
class ProfileTests(unittest.TestCase):
    ...
    def test_predicates_and_authoritative_mappings_cannot_be_weakened(self):
        edits = [lambda c: c.update(requiredCleanBuilds=1), lambda c: c.update(networkDuringVerification=True),
                 ...
                 lambda c: c["phases"][0].update(scope="universal unbounded correctness"),
    ...
    def test_source_rejection_cannot_mask_compilation_or_loader_failure(self):
        ...
        profile.check_rejection("build1-source-reject-records-field", good)
        for bad in ("compile failed", good + "Example.java:1: error: cannot find symbol\n", good + "unexpected output\n"):
            with self.assertRaises(previous.Blocked):
                profile.check_rejection("build1-source-reject-records-field", bad)
...
class DispatcherTests(unittest.TestCase):
    ...
    def test_error_is_never_replaced_with_a_successful_replay(self):
        for error in (previous.Blocked("missing observation"), TypeError("encoder bug"), SystemExit(0)):
            with patch.object(self.plugins["records"], "generate", side_effect=error), self.assertRaises(type(error)):
                self.dispatcher.generate(Path("build"), Path("formal"))

    def test_names_counts_and_namespace_ownership_are_exact(self):
        area = replay.AREAS[1]
        for record in [(area["replay"], theorem("Fixture.law"), 1),
                       (area["replay"], theorem("Fixture.records"), 2),
                       ("Foreign.lean", theorem("Fixture.records"), 1)]:
            with patch.object(self.plugins["records"], "generate", return_value=[record]), \
                    self.assertRaises(previous.Blocked):
                self.dispatcher.generate(Path("build"), Path("formal"))

    def test_source_mutation_stays_with_its_registered_extractor(self):
        original = self.plugins["law"].source_mutations()
        for values in (original * 2, [(*original[0][:4], "CanonicalWireTablesExtractor")]):
            with patch.object(self.plugins["law"], "source_mutations", return_value=values), \
                    self.assertRaises(previous.Blocked):
    ...
    def test_missing_area_file_is_blocking(self):
        dispatcher = replay.load_dispatcher()
        with tempfile.TemporaryDirectory() as directory:
            dispatcher.__file__ = str(Path(directory) / "fifth_obligation_replays.py")
            for area in replay.AREAS:
                with self.assertRaises(previous.Blocked):
