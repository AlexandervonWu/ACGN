#!/usr/bin/env python3
"""Frozen v2.15 profile of the unchanged two-clean-build closure engine.
...
from fourth_obligation_replays import AREAS

PACKAGE = Path("docs/obligation-repair/fourth-five")
CONFIG = PACKAGE / "closure-config.json"
PLUGIN = "scripts/fourth_obligation_replays.py"
BASE_SHA256 = "d3006d1b0e093cd7fc5e9c277c19a36e0828329dc6c4f3cacff7201ef8c48f55"
PARENT_HASHES = {
    "P2-20": "96b8ce07afc0aad611a36396786a36cd235e5c553f40611962d168721e70b936",
    "P2-18": "4dff4190a5709dd2253c47a941bd934e2b276bd595b7d9a4a581fd647b772735",
    "A2-07": "1f73b67ab4d51c8aa6cea982b564d710f2a6711852633153eedd075026a87f6e",
    "A2-11": "d12228a367db5e5046a6eb901b867b559db8c57590b6c91ada4e8dd473459ef5",
    "P3-03": "c5733bd96ae6281cc4f3f9fe6d51e66261c20b0b0bf4b3aa426aaa210c7b0fca",
}
PARENTS = frozenset(PARENT_HASHES)
PROOF_DEPENDENCIES = ()
NOTES = ("container-notes.md", "chain-notes.md", "profile-notes.md", "harness-notes.md")
AREA_TESTS = tuple("scripts/test_" + area["plugin"] for area in AREAS)
REQUIRED_INPUTS = frozenset({
    "scripts/run_fourth_obligation_repairs.py", "scripts/test_fourth_obligation_repairs.py",
...
def check_rejection(label, content):
    ...
    source = re.search(r"(?:^|-)source-reject-(container|chain|profile)-", label)
    if source:
        ...
        valid = bool(re.search(r"(?:^|-)reject-(container|chain|profile)-", label))
...
LITERALS = {
    ...
    "scripts/test_next_obligation_repairs.py": ("scripts/test_fourth_obligation_repairs.py", 2),
    "[next-five] ": ("[fourth-five] ", 1),
    "_next_obligation_replays_": ("_fourth_obligation_replays_", 1),
    "next-five-v2.13-": ("fourth-five-v1-", 1),
...
def load_runner():
    ...
    module = ModuleType("_fourth_five_engine")
    ...
    def validate_config(config):
        ...
        require(config.get("profile") == "fourth-five-v1" and config.get("replayPlugin") == PLUGIN,
                "unknown fourth-five profile/plugin")
        require(REQUIRED_INPUTS <= set(config.get("inputs", [])), "missing fourth-five dependency")
        require(config.get("proofDependencies") == list(PROOF_DEPENDENCIES),
                "fourth-five proof dependencies changed", "UNDECLARED_DEPENDENCY")
        for phase in config["phases"]:
            parent = phase["parent"]
            area = next(area for area in AREAS if parent in area["parents"])
            require(phase["id"] == "FF-" + parent and phase["claimSha256"] == PARENT_HASHES[parent],
