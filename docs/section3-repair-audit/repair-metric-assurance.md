...
| M-06 | Charge one for a matrix head update and the full size of an inserted/deleted operand subtree; use assignment only at certified unordered containers. | `matrixOperandDeletionRemovesTheWholeSubtree`. | Sequence/bag/set vectors and `A(X)` to `X = 2`. | `DIRECT`: general recurrence proof open. |
