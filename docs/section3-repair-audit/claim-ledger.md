...
| P1-10 | U/I | Zero-argument calls retain callee, arity zero, and END. | `zero_argument_call_is_valid` | Zero-argument local/imported fixtures | `PARTIAL/DIRECT` |
...
| A2-06 | F/I | The implementation's JOIN guard accepts binary interiors and rejects unary interiors. | `unary_interior_join_has_no_flat_license`; `binary_interior_join_has_flat_license` | Theory and pipeline probes | `PARTIAL/DIRECT` |
...
| A2-11 | P | Certificate identity binds kind, profile, source association, exact operand types, boundary correspondences, result, target, theory version, and digest. | `ChainIndex` and complete-index field injectivity | One-index-at-a-time replay attacks | `PARTIAL/PARTIAL` |
...
| P2-02 | U/I | `ArityPolicy`, `SiblingQuotient`, `FlatLicense`, and `UnitLicense` are represented independently. | Policy product/separation theorem | Java API construction matrix | `PARTIAL/DIRECT` |
...
| P2-05 | U/I | A flat port has exactly one root-container source argument. | Flat-source schema theorem | Multi-port/cross-argument flatten attacks | `PARTIAL/DIRECT` |
...
| P3-03 | P | Profile fingerprint binds bitwidth, overflow, temporal/rewrite modes, and exact signature version. | `profile_payload_is_injective`; cryptographic/encoding refinement absent | One-field profile mutations | `PARTIAL/DIRECT` |
