...
| P5-43 | U/I/P | An outer lexical binder is imported separately into every unary and left/right binary temporal phase under exact source-lineage, owner-phase, binder-context, coordinate, and target-path evidence; detached uses and mismatched provenance reject, and unsafe empty-carrier conjunction prenexing remains local. | Temporal-vocabulary, lexical-depth, phase-import, provenance-rejection, and empty-carrier counterexample theorems in `TemporalPhaseLocalBinding.lean` | Sibling-phase reuse, alpha-renaming, changed-use, repeated-snapshot, and malformed-provenance regressions | `FORMAL-PARTIAL` |
