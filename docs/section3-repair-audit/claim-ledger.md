...
| P6-02 | U/I | Orbit candidates are streamed and only the least `(shape,witness)` under one declared order is retained. | Streaming/reference equivalence theorem | Instrumented nonmaterialization and differential probe | `BLOCKED` |
...
| P6-10 | U/I | Neither global nor local orbit enumeration retains the orbit merely to claim streaming or linear workspace. | Stream/materialization refinement theorem | Retained-object instrumentation, including `S7` | `BLOCKED` |
| P6-11 | P | The current rooted-binder, CALL-provenance, exact-transition, and witness-unfold contract is admitted only under schema v8; schema v5 rootless, schema v6 provenance-incomplete, and schema v7 transition-incomplete bytes are never reinterpreted. | Version-disjointness theorem in `Phase4CollisionBuckets.lean`, rooted-key theorem in `Phase6OrbitCanonicalization.lean`, and definitional witness theorem in `Phase4WitnessUnfold.lean` | Otherwise-valid v5/v6/v7 relabel mutations and v8 two-root FULL replay | `FORMAL-PARTIAL` |
...
| M-02 | U/I | Quantifier insertion, deletion, or modification of the complete `(quantifier,type,cardinality,disjointness-class)` tuple costs one unit. | Quantifier edit/update theorems in `RepairMetricSemantics.lean` | Exhaustive finite tuple-operation probe | `FORMAL-PARTIAL` |
| M-03 | U/I | Pairwise alpha alignment is the exact minimum over all and only maximum-cardinality certified compatible mappings. | `alpha_distance_is_exact_pairwise_minimum`; proof-carrying `CertifiedAlphaAlignment`; `nonmaximum_empty_alignment_is_not_admissible` | Brute-force mapping differential, including scope/exchange rejection | `FORMAL-PARTIAL` |
| M-04 | U/I | ACI child comparison is exact minimum-cost assignment including insertion/deletion alternatives, not positional sorting. | `aci_distance_is_exact_minimum_cost_assignment`; proof-carrying left/right partition in `CertifiedAssignment`; crossed-pair witness | Hungarian-versus-brute-force differential | `FORMAL-PARTIAL` |
...
| M-06 | U/I | Matrix edit units count node insertions, deletions, and one-unit semantic head updates over certified quotient operands. | Matrix recurrence theorem | Independent small-e-graph enumerator | `BLOCKED` |
...
| M-08 | I | Search pruning, candidate ordering, and Hungarian implementation preserve the exact minima above. | Algorithm refinement/invariant proofs | Exhaustive bounded algorithm differential | `BLOCKED` |
