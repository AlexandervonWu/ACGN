...
public final class ContainerWitnessTransitionsRegressionTest {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            ...
            pipelineCases();
            for (int family : List.of(0, 2, 3)) wireCases(family);
        } finally {
        ...
        check(ROWS.size() == 1598, "1597 frozen observations, got " + (ROWS.size() - 1));
    ...
    private static void wireCases(int family) throws Exception {
        ...
        if (family == 3) {
            ...
            Tree tree = Tree.node(Tree.leaf(1), Tree.node(Tree.leaf(0), Tree.node(Tree.leaf(1), Tree.leaf(0))));
        ...
        CertificateProvenance provenance = CertificateProvenance.capture("fixture/container-witness/" + family,
                ("container-witness-" + family).getBytes(StandardCharsets.UTF_8), "container-witness-test-v1;" + CertificateTheoryManifest.VERSION);
        ...
        VerificationPolicy policy = VerificationPolicy.trust(Bundle.parse(original).theoryDigest())
                .withCallOccurrenceCommitment(CallOccurrenceCommitment.inspect(bytes, Limits.defaults()));
        IndependentVerifier verifier = new IndependentVerifier();
        var positive = verifier.verify(bytes, org.acgn.cert.Profile.FULL, policy);
        check(positive.outcome().toString().equals("VERIFIED") && positive.code() == FailureCode.NONE, "FULL original: " + positive);
        ...
        for (String label : mutations) {
            ...
            var result = verifier.verify(candidate, org.acgn.cert.Profile.FULL, policy);
            check(result.outcome().toString().equals("REJECTED") && (result.code() == FailureCode.THEORY_MISMATCH
                    || result.code() == FailureCode.INVALID_RECORD_SHAPE), "exact semantic/record control " + label + ": " + result);
